...
export default function Header({
  ...
  color: { headerColor: string };
}): React.JSX.Element {
  const { data: session } = useSession() as { data: UserSession };
  const router = useRouter();
  const headerOpacity = useRecoilState(HeaderAtomState)[0];
  const [playlistHistory, setPlaylistHistory] = useRecoilState(
    playlistHistoryAtomState
  );
  const [playlistHistoryIndex, setPlaylistHistoryIndex] = useRecoilState(
    playlistHistoryIndexAtomState
  );
  ...
  return (
    <div
      className={`sticky top-0 z-50 flex flex-row items-center justify-between p-3 ${headerOpacity} backdrop-filter backdrop-blur-md   ${
    ...
    >
      <div className="hidden flex-row items-center space-x-4 px-5 sm:flex">
        ...
        <ChevronRightIcon
          className={`h-8 w-8 cursor-pointer rounded-full bg-black bg-opacity-50 p-1 text-gray-200 transition duration-300 hover:bg-opacity-75 hover:text-white ${
            playlistHistoryIndex === playlistHistory.length - 1
              ? "opacity-50"
              : ""
          }`}
          onClick={(): void => {
      ...
      <div className="flex-grow flex justify-center items-center ms-14">
        <LanguageSelector />
      </div>

      <div className="flex flex-row items-center space-x-4 px-5 sm:hidden">
        <HomeIcon
          className="h-8 w-8 cursor-pointer rounded-full glass p-2 text-gray-200 transition duration-300 hover:bg-opacity-75 hover:text-white"
          onClick={(): void => {
            setSearch(false);
            setPlaylistID("");
